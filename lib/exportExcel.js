...
function dlXlsx(XLSX, wb, fname) {
  ...
  fd.type = 'hidden'; fd.name = 'data'; fd.value = b64;
  const fn = document.createElement('input');
  fn.type = 'hidden'; fn.name = 'name'; fn.value = fname;
  form.append(fd, fn);
...
export async function exportHemato({
  ...
  const _xl = await import('xlsx'); const XLSX = _xl.default ?? _xl;
  const d = todayStr();
  ...
  const cprrBotIdx = aoa.findLastIndex(row => String(row[0]).startsWith('CPRR (margin'));

  const ws = applySetup(XLSX.utils.aoa_to_sheet(aoa), [34, 28, 20, 22]);
  if (cprrTopIdx >= 0) boldRow(ws, cprrTopIdx, 4);
  if (cprrBotIdx >= 0) boldRow(ws, cprrBotIdx, 4);
